package basedatos.movil.meteoreport.receivers
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        btnSync.setOnClickListener {
            if (!isNetworkAvailable()) {
                Toast.makeText(requireContext(), "❌ Sin conexión a internet", Toast.LENGTH_SHORT)
                    .show()
            ...
            CoroutineScope(Dispatchers.IO).launch {
                try {
                    ...
                    android.util.Log.d("SYNC", "Pendientes encontrados: ${pending.size}")
                    for (report in pending) {
                        android.util.Log.d("SYNC", "Sincronizando: ${report.city}")
                        ...
                        android.util.Log.d("SYNC", "Sincronizado: ${report.city}")
                    }
                } catch (e: Exception) {
                    android.util.Log.e("SYNC", "Error: ${e.message}")
                    e.printStackTrace()
                }
            }
        }
